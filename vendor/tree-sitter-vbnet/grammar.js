...
module.exports = grammar({
  ...
  rules: {
    ...
    type_parameters: ($) => seq(kw("Of"), commaSep1($.type_parameter)),
    type_parameter: ($) =>
      seq(
        field("name", $.identifier),
    ...
    attribute_block: ($) =>
      seq("<", commaSep1($.attribute), ">", $._terminator),
    // attribute: $ => seq(
    ...
    parameter: ($) =>
      seq(
        ...
        optional(
          choice(kw("Optional"), kw("ByVal"), kw("ByRef"), kw("ParamArray")),
        ),
        field("name", $.identifier),
        optional($.array_rank_specifier),
        optional($.as_clause),
    ...
    left_hand_side: ($) =>
      choice($.identifier, $.member_access, $.element_access),

    call_statement: ($) =>
      seq(choice(seq(kw("Call"), $.expression), $.expression), $._terminator),
        ...
        prec(
          ...
          seq(
            ...
            optional(
              seq(
                kw("Else"),
                field("else_branch", choice($.statement, $.expression)),
              ),
            ),
          ),
        ),
    ...
    else_clause: ($) => seq(kw("Else"), $._terminator, repeat($.statement)),
    ...
    case_else_block: ($) =>
      seq(kw("Case"), kw("Else"), $._terminator, repeat($.statement)),
    case_clause: ($) =>
      choice(
        $.expression,
    ...
    finally_block: ($) =>
      seq(kw("Finally"), $._terminator, repeat($.statement)),
    ...
    return_statement: ($) =>
      seq(kw("Return"), optional($.expression), $._terminator),
    ...
    continue_statement: ($) =>
      seq(
        kw("Continue"),
        choice(kw("Do"), kw("For"), kw("While")),
        $._terminator,
      ),

    throw_statement: ($) =>
      seq(kw("Throw"), optional($.expression), $._terminator),

    goto_statement: ($) =>
      seq(kw("GoTo"), field("label", $.identifier), $._terminator),
    ...
    argument_list: ($) => seq("(", optional(commaSep($.argument)), ")"),
    argument: ($) =>
      choice(
        $.expression,
    ...
    new_expression: ($) =>
      seq(
        ...
        field("type", $.type),
        optional($.argument_list),
        optional(choice($.object_initializers, $.with_initializer)),
      ),

    with_initializer: ($) =>
      seq(kw("With"), "{", optional(commaSep($.member_initializer)), "}"),
    ...
    object_initializers: ($) =>
      seq("{", optional(commaSep($.object_initializer)), "}"),
    object_initializer: ($) =>
      choice(
