...
//! `TYPESAFE_API_KEY`, `AI_GATEWAY_API_KEY`, or `OPENROUTER_API_KEY` in the
//! environment and spend a fraction of a cent.
...
fn evaluate_twice(provider: JevProvider, namespace: &str, key: &str) {
    if std::env::var_os(key).is_none() {
        eprintln!("skipping: set {key}");
        return;
    }
...
fn typesafe_answers_and_caches_the_seven_questions() {
    evaluate_twice(JevProvider::TypeSafe, "jev", "TYPESAFE_API_KEY");
}

#[test]
#[ignore = "live Vercel AI Gateway call; requires AI_GATEWAY_API_KEY"]
fn vercel_answers_and_caches_the_seven_questions() {
    evaluate_twice(JevProvider::Vercel, "jev-vercel", "AI_GATEWAY_API_KEY");
}

#[test]
#[ignore = "live OpenRouter call; requires OPENROUTER_API_KEY"]
fn openrouter_answers_and_caches_the_seven_questions() {
    evaluate_twice(
