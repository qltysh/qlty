...
impl RuleSummary {
    ...
    ///
    /// `ln(1 + x)` is computed as `(x + 1).ln()` rather than `x.ln_1p()`
    /// because glibc's `log1p` differs from the macOS reference even for
    /// small counts (`log1p(2)` is 1 ULP below `ln 3`). Counts and Qlty's
    /// actual measurements are integers, so the addition is exact, and `log`
    /// on macOS, glibc, and musl agrees for every integer below 19142.
...
pub(crate) fn config() -> &'static QltyConfig {
