...
impl Model {
    ...
    /// The exact additive score change along the straight `before` to
    /// `after` feature path, per feature.
    ///
    /// A logistic component has one linear predictor. Integrating its
    /// derivative along this path yields a secant slope times each weighted
    /// feature change. The original attributions are rescaled between the two
    /// endpoint scores, so the results sum to `predict(after) - predict(before)`.
    /// Constant components and unchanged features contribute zero. This is an
    /// exact port of slopdetect's `weekly_explanations.feature_changes`.
    pub fn explain_between(&self, before: &[f64], after: &[f64]) -> Result<Vec<f64>> {
        let size = self.reference.len();
        if before.len() != size || after.len() != size {
            return Err(Error::InvalidInput);
        }
        if !before.iter().chain(after).all(|value| value.is_finite()) {
            return Err(Error::InvalidInput);
        }

        let count = self.components.len();
        let mut slopes: Vec<(&[f64], f64)> = Vec::with_capacity(count);
        let mut old_risks = Vec::with_capacity(count);
        let mut new_risks = Vec::with_capacity(count);
        for component in &self.components {
            match component {
                Component::Constant(constant) => {
                    old_risks.push(*constant);
                    new_risks.push(*constant);
                }
                Component::Logistic { weights, intercept } => {
                    let start = intercept + fsum(weights.iter().zip(before).map(|(w, x)| w * x));
                    let change = fsum(
                        weights
                            .iter()
                            .zip(before)
                    ...
                    let old_risk = sigmoid(start);
                    let new_risk = sigmoid(start + change);
                    old_risks.push(old_risk);
                    new_risks.push(new_risk);
                    let slope = if change.abs() > 1e-8 {
                        (new_risk - old_risk) / change
                    } else {
                        let middle = sigmoid(start + change / 2.0);
                        middle * (1.0 - middle)
                    };
                    slopes.push((weights, slope));
                }
            }
        }

        let divisor = count as f64;
        let multiplier = self.score_scale.secant(
            10.0 - 9.0 * fsum(old_risks) / divisor,
            10.0 - 9.0 * fsum(new_risks) / divisor,
        );
        Ok(before
            .iter()
            .zip(after)
            ...
            .map(|(index, (a, b))| {
                -9.0 * multiplier
                    * (b - a)
                    * fsum(slopes.iter().map(|(weights, slope)| weights[index] * slope))
                    / divisor
            })
            .collect())
    }
...
mod tests {
    ...
    fn random_model(rng: &mut Lcg) -> Model {
        let components: Vec<Value> = (0..64)
            .map(|_| json!({"weights": rng.vector(-3.0, 3.0), "intercept": rng.uniform(-3.0, 3.0)}))
            .chain([json!({"constant": 0.4})])
            .collect();
        Model::try_new(document_with(json!({
            "reference": rng.vector(0.0, 1.0),
            "components": components,
        })))
        .unwrap()
    ...
    #[test]
    fn explain_between_identical_vectors_is_zero() {
        let mut rng = Lcg(7);
        let model = random_model(&mut rng);
        let values = rng.vector(0.0, 5.0);
        let changes = model.explain_between(&values, &values).unwrap();
        assert_eq!(changes, vec![0.0; feature_names().len()]);
    }

    #[test]
    fn explain_between_sums_to_the_score_change() {
        let mut rng = Lcg(11);
        let model = random_model(&mut rng);
        let residuals: Vec<f64> = (0..20)
            .map(|_| {
                let before = rng.vector(0.0, 5.0);
                let after = rng.vector(0.0, 5.0);
                let expected =
                    model.predict(&after).unwrap().score - model.predict(&before).unwrap().score;
                fsum(model.explain_between(&before, &after).unwrap()) - expected
            })
            .collect();
        assert!(residuals.iter().all(|residual| residual.abs() < 1e-9));
    }

    #[test]
    fn explain_between_from_the_reference_matches_predict_contributions() {
        let mut rng = Lcg(13);
        let model = random_model(&mut rng);
        let values = rng.vector(0.0, 5.0);
        let prediction = model.predict(&values).unwrap();
        let changes = model.explain_between(model.reference(), &values).unwrap();
        assert!(changes
            .iter()
            .zip(&prediction.feature_contributions)
            .all(|(change, contribution)| (change - contribution).abs() < 1e-9));
    }

    #[test]
    fn explain_between_reverses_with_the_comparison() {
        let mut rng = Lcg(17);
        let model = random_model(&mut rng);
        let before = rng.vector(0.0, 5.0);
        let after = rng.vector(0.0, 5.0);
        let forward = model.explain_between(&before, &after).unwrap();
        let reverse = model.explain_between(&after, &before).unwrap();
        assert!(forward
            .iter()
            .zip(&reverse)
    ...
    #[test]
    fn explain_between_rejects_wrong_length_or_non_finite_input() {
        let model = Model::try_new(document()).unwrap();
        let values = vec![0.0; feature_names().len()];
        assert!(matches!(
            model.explain_between(&values, &[]),
            Err(Error::InvalidInput)
        ));
        let nan = vec![f64::NAN; feature_names().len()];
        assert!(matches!(
            model.explain_between(&values, &nan),
            Err(Error::InvalidInput)
        ));
    }
