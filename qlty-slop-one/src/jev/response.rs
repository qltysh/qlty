...
/// OpenRouter's dated slug for the build TypeSafe names [`JEV_MODEL`].
const OPENROUTER_JEV_MODEL: &str = "typesafe/jev-1.13-20260917";
...
/// The model a provider must report, and its spelling of the boolean answer.
    ...
    fn for_provider(provider: JevProvider) -> Self {
        match provider {
            JevProvider::TypeSafe => Self {
                model: Some(JEV_MODEL),
                boolean_type: "noul",
                boolean_key: "noul",
            },
            JevProvider::OpenRouter => Self {
                model: Some(OPENROUTER_JEV_MODEL),
                boolean_type: "noul",
                boolean_key: "noul",
            },
...
mod tests {
    ...
    fn openrouter_response() -> Value {
        let mut response = typesafe_response();
        response["model"] = json!("typesafe/jev-1.13-20260917");
        response["id"] = json!("gen-dec-1790122401-QZ7nXbT4kWm2Hs9LpR3c");
        response["provider"] = json!("TypeSafe");
        response["usage"]["cost"] = json!(0.000267);
        response
    }
    ...
    #[test]
    fn accepts_an_openrouter_response_for_the_openrouter_provider() {
        let typesafe = validate(&typesafe_response(), JevProvider::TypeSafe).unwrap();
        let openrouter = validate(&openrouter_response(), JevProvider::OpenRouter).unwrap();
        assert_eq!(typesafe, openrouter);
    }

    #[test]
    fn openrouter_provider_rejects_the_typesafe_model_name() {
        assert_eq!(
            message(validate(&typesafe_response(), JevProvider::OpenRouter)),
            "Jev returned an unexpected model version."
        );
    }

    #[test]
    fn typesafe_provider_rejects_the_openrouter_model_name() {
        assert_eq!(
            message(validate(&openrouter_response(), JevProvider::TypeSafe)),
            "Jev returned an unexpected model version."
        );
    }
