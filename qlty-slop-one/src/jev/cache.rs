...
//! `{"key": key, "response": <raw provider body>}`. Vercel and OpenRouter
//! responses have different shapes and live apart, under
//! `<cache_dir>/jev-vercel/` and `<cache_dir>/jev-openrouter/`.
...
impl Cache {
    pub fn new(cache_dir: &Path, provider: JevProvider) -> Self {
        let namespace = match provider {
            ...
            JevProvider::OpenRouter => "jev-openrouter",
        };
        Self {
...
mod tests {
    ...
    #[test]
    fn openrouter_responses_live_under_their_own_directory() {
        let dir = tempfile::tempdir().unwrap();
        Cache::new(dir.path(), JevProvider::OpenRouter)
            .write(KEY, &json!({"model": "typesafe/jev-1.13-20260917"}))
            .unwrap();
        assert!(dir
            .path()
            .join("jev-openrouter")
