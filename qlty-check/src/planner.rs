...
impl Planner {
    ...
    fn build_plan(&mut self) -> Result<Plan> {
        ...
        Ok(Plan {
            verb: self.verb,
            target_mode,
            progress_enabled: self.settings.progress,
            install_only: self.settings.install_only,
            skip_errored_plugins: self.settings.skip_errored_plugins,
            ai_enabled: self.settings.ai,
