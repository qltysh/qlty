...
fn configure_plugins(planner: &Planner) -> Result<Vec<ActivePlugin>> {
    ...
    let merged_unique_enabled_plugins =
        compute_unique_merged_enabled_plugins(&planner.config.plugin);

    for enabled_plugin in merged_unique_enabled_plugins {
...
fn merge_enabled_plugins(existing: &EnabledPlugin, new: &EnabledPlugin) -> EnabledPlugin {
    ...
    if existing.version != new.version {
        warn!(
            ...
            existing.version, new.version, existing.version
        );
    }
...
mod test {
    ...
    use qlty_config::{
        config::{CheckTrigger, DriverDef, ExtraPackage, PluginFetch, PluginsConfig},
    ...
    fn test_compute_unique_merged_enabled_plugins_no_duplicates() {
        ...
        assert_eq!(result[1].name, "plugin2");
