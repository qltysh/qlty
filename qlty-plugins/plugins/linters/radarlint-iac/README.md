...
```bash
qlty plugins enable radarlint-iac
```
...
radarlint-iac analyzes: Dockerfiles and Terraform HCL files (`.tf`, `.tfvars`).

## Configuration files

- `radarlint.properties`

To keep your project tidy, you can move configuration files into `.qlty/configs` and Qlty will find and use them when running radarlint-iac.
...
- [radarlint-iac plugin definition](https://github.com/qltysh/qlty-plugins/tree/main/plugins/linters/radarlint-iac)
