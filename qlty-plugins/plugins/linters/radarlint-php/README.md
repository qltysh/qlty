...
radarlint-php is a static analysis tool for PHP code, running as a language-specific mode of the radarlint engine.

<!-- REVIEW: add upstream docs URL if radarlint has a public home page -->

## Enabling radarlint-php

Enabling with the `qlty` CLI:

```bash
qlty plugins enable radarlint-php
```
...
radarlint-php will be automatically enabled by `qlty init` if a `radarlint.properties` configuration file is present.

## Languages and file types

radarlint-php analyzes: PHP (`.php`).

## Configuration files

- `radarlint.properties`

To keep your project tidy, you can move configuration files into `.qlty/configs` and Qlty will find and use them when running radarlint-php.
...
- [radarlint-php plugin definition](https://github.com/qltysh/qlty-plugins/tree/main/plugins/linters/radarlint-php)
